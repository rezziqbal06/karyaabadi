...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		$().btnSubmit();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/pengaturan/produk/")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						$.get('<?=base_url("api_admin/pengaturan/produk/detail/")?>'+ieid).done(function(dt){
							if(dt.data){
						...
						$("#adetail").attr("href","<?=base_url_admin("pengaturan/produk/detail/")?>"+ieid);
						$("#aedit").attr("href","<?=base_url_admin("pengaturan/produk/edit/")?>"+ieid);
...
$("#fedit").on("submit",function(e){
	...
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/pengaturan/produk/edit/")?>';
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/pengaturan/produk/hapus/')?>'+ieid;
...
$("#fl_a_company_id_parent").select2({
	ajax: {
		method: 'post',
		url: '<?=base_url("api_admin/pengaturan/produk/get_parent/")?>',
		dataType: 'json',
    delay: 250,
...
$("#atambah").on("click",function(e){
		e.preventDefault();
		window.location = '<?=base_url_admin('pengaturan/produk/baru/')?>'
	});

// edit modal
$("#aedit").on("click",function(e){
	e.preventDefault();
	window.location = '<?=base_url_admin('pengaturan/produk/edit/')?>' + ieid
});
