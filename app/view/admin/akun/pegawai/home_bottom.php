...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		$().btnSubmit();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/akun/pegawai/")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						$.get('<?=base_url("api_admin/akun/pegawai/detail/")?>'+ieid).done(function(dt){
							if(dt.data){
								if(dt.data.utype){
						...
						$("#adetail").attr("href","<?=base_url_admin("akun/pegawai/detail/")?>"+ieid);
						$("#aedit").attr("href","<?=base_url_admin("akun/pegawai/edit/")?>"+ieid);
						$("#amodule").attr("href","<?=base_url_admin("akun/pegawai/module/")?>"+ieid);
						$("#areseller").attr("href","<?=base_url_admin("partner/reseller/baru/")?>"+ieid);
						$("#ieid-pegawai").val(ieid);
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			var url = '<?=base_url('api_admin/akun/pegawai/hapus/')?>'+ieid;
...
$("#atambah").on("click",function(e){
		e.preventDefault();
		window.location = '<?=base_url_admin("akun/pegawai/baru/")?>'
	});
